...
module.exports = {
  ...
  execute: async (interaction, user, bypassChecks = false) => {
    try {
      ...
      if (!discordUsername && bypassChecks !== true) {
        throw new HypixelDiscordChatBridgeError("This player does not have a Discord linked.");
      }

      if (discordUsername?.toLowerCase() !== interaction.user.username.toLowerCase() && bypassChecks !== true) {
        throw new HypixelDiscordChatBridgeError(
      ...
      console.log(error);
      error = error.toString().replaceAll("Error: [hypixel-api-reborn] ", "").replaceAll(
